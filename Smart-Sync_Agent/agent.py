...
# Import the tool loader needed for stateful sessions
...
async def run_agent_workflow(user_input_text: str, gemini_key: str, root_dir: str):
    ...
    # Open a persistent, stateful session with the client
    async with client.session("cli_mcp") as session:
        # Load the tools tied explicitly to this active session
        ...
        async def tool_calling_model(state: State):
            # Reduced the 20s artificial sleep to avoid terminal hanging
